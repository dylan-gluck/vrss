...
import type { ErrorCode, RPCRequest, RPCResponse } from "@vrss/api-contracts";
...
describe("Type Contracts", () => {
  ...
  describe("ErrorCode Enum", () => {
    it("should have authentication error codes (1000-1099)", () => {
      ...
      const authCodes = ["UNAUTHORIZED", "INVALID_CREDENTIALS", "SESSION_EXPIRED", "INVALID_TOKEN"];
    ...
    it("should have resource error codes (1300-1399)", () => {
      const resourceCodes = ["NOT_FOUND", "RESOURCE_NOT_FOUND", "USER_NOT_FOUND", "POST_NOT_FOUND"];
  ...
  describe("Procedure Namespaces", () => {
    it("should define AuthProcedures namespace", () => {
      ...
      const authProcedures = ["auth.register", "auth.login", "auth.getSession", "auth.logout"];

      for (const proc of authProcedures) {
    ...
    it("should define PostProcedures namespace", () => {
      ...
      for (const proc of postProcedures) {
        expect(proc).toBeDefined();
      }
    });
  });
  ...
  describe("Shared Domain Types", () => {
    ...
    it("should define CustomFeed type", () => {
      const requiredFields = ["id", "userId", "name", "filters", "createdAt", "updatedAt"];
