...
describe('Docker Network', () => {
  it('should have vrss_network created', () => {
    const networkOutput = execSync('docker network ls').toString();
    expect(networkOutput).toContain('vrss_network');
  }, TIMEOUT);

  it('should have all services connected to vrss_network', () => {
    const networkInspect = execSync('docker network inspect vrss_network --format json').toString();
...
describe('Service-to-Service Communication', () => {
  it('should allow backend to reach database by service name', async () => {
    try {
      // Try to ping database from backend using service discovery
      const output = execSync(
        'docker-compose exec -T backend sh -c "getent hosts db"',
        { timeout: 10000 }
      ).toString();
  ...
  it('should allow backend to connect to database on port 5432', async () => {
    try {
      // Check if backend can connect to database
      const output = execSync(
        'docker-compose exec -T backend sh -c "nc -zv db 5432 2>&1"',
        { timeout: 10000 }
      ).toString();

      // netcat output varies, check for success indicators
      const success = output.includes('open') ||
  ...
  it('should allow frontend to reach backend by service name', async () => {
    try {
      const output = execSync(
        'docker-compose exec -T frontend sh -c "getent hosts backend"',
        { timeout: 10000 }
      ).toString();
  ...
  it('should have containers on same network subnet', () => {
    const networkInspect = execSync('docker network inspect vrss_network --format json').toString();
    const network = JSON.parse(networkInspect)[0];
...
describe('Port Exposure', () => {
  ...
  it('should allow host to connect to backend', async () => {
    try {
  ...
  it('should allow host to connect to frontend', async () => {
    try {
